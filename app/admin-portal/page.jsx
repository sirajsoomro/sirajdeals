...
const Portal = () => {
  ...
  const allDeta = async () => {
    ...
    try {
      let uploadedUrls = previewUrls;

      if (imageFiles.length > 0) {
        uploadedUrls = await uploadToCloudinary(imageFiles);
      ...
      if (!editProductId && uploadedUrls.length === 0) {
      ...
      <div className='bg-white w-full '>
        ...
        <div className={`fixed inset-0 bg-black/30 backdrop-blur-md flex items-center justify-center ${isOpen ? "flex" : "hidden"}`}>
          <div className='bg-white p-6 rounded-lg shadow-lg w-1-2 px-20'>
            ...
            <label htmlFor="images" className="mt-4 block font-bold text-gray-700">Upload Images</label>
            <input
              type="file"
              id='images'
              accept="image/*"
            ...
            {previewUrls.length > 0 && (
              <div className="mt-2 flex gap-2 flex-wrap">
                {previewUrls.map((url, index) => (
                  <img key={index} src={url} alt={`Preview ${index}`} className="h-20 w-auto rounded-md" />
                ))}
              </div>
            )}
          ...
          <tbody>
            {products.length >= 1 ? (
              products.map((product) => (
                <tr key={product.id}>
                  <td className="p-3 border">
                    <img src={(product.images && product.images[0]) || "watch.png"} alt="Product" className="cursor-pointer rounded-md m-auto h-11 w-10" />
                  </td>
                  <td className="p-3 border">{product.name}</td>
                  <td className="p-3 border">{product.price}</td>
                  <td className="p-3 border">{product.description}</td>
